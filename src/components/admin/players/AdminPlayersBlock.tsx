          ...
          filteredPlayers.map((player) => (
            ...
            >
              <div className="flex items-center gap-3">
                {player.imageUrl ? (
                  <img src={player.imageUrl} alt={player.name} className="size-10 object-cover rounded-full bg-pitch-950 border border-slate-700 group-hover:border-slate-500 transition-colors" />
                ) : (
                ...
                <div>
                  <div className="font-semibold text-slate-200 text-sm">{player.name}</div>
                  <div className="text-slate-500 text-xs mt-0.5">{player.position1} {player.position2 && ` • ${player.position2}`}</div>
                </div>
              </div>
              <div className="text-right">
                <div className={`text-xs font-mono font-bold ${
                  player.status === "SOLD" ? "text-accent-green" :
                  player.status === "UNSOLD" ? "text-slate-500" :
                  "text-accent-gold"
                }`}>
                  {player.status}
                </div>
                {player.status === "SOLD" && player.transactionAmount && (
            ...
            <div className="flex flex-col">
              ...
              <div className="relative w-full h-72 bg-pitch-950 border-b border-slate-800 flex items-center justify-center overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  <div className={`px-3 py-1.5 rounded-md border text-xs font-bold tracking-widest uppercase shadow-sm backdrop-blur-md ${
                     selectedPlayer.status === "SOLD" ? "bg-accent-green/20 text-accent-green border-accent-green/30" :
                     selectedPlayer.status === "UNSOLD" ? "bg-slate-800/80 text-slate-400 border-slate-700" :
              ...
              <div className="p-6 pt-0 flex flex-col gap-6 relative z-10 -mt-8">
                <div>
                    <h2 className="text-3xl font-bold font-heading text-slate-100">{selectedPlayer.name}</h2>
                    <p className="text-sm text-accent-gold mt-1 font-medium tracking-wide">{selectedPlayer.position1} {selectedPlayer.position2 && ` • ${selectedPlayer.position2}`}</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-pitch-950/50 rounded-xl p-4 border border-slate-800/50 flex flex-col justify-center">
                    ...
                    <p className="text-xl font-mono font-bold text-slate-200">{selectedPlayer.basePrice.toLocaleString()}</p>
                  </div>
                  {selectedPlayer.transactionAmount ? (
                    <div className="bg-pitch-950/50 rounded-xl p-4 border border-slate-800/50 flex flex-col justify-center">
                        <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Sold For</p>
                        <p className="text-xl font-mono font-bold text-accent-green">{selectedPlayer.transactionAmount.toLocaleString()}</p>
                    </div>
                  ) : (
                    <div className="bg-pitch-950/50 rounded-xl p-4 border border-slate-800/50 flex flex-col justify-center">
                        <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Status</p>
                        <p className="text-lg font-mono font-bold text-slate-400">{selectedPlayer.status === 'UNSOLD' ? 'Unsold' : 'In Auction'}</p>
                    </div>
                  )}
                  {selectedPlayer.teamId && selectedPlayer.team ? (
                    <div className="bg-pitch-950/50 rounded-xl p-4 border border-slate-800/50 col-span-2 flex items-center justify-between">
                        <div>
                           <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Team Details</p>
                           <p className="font-medium text-slate-100">{selectedPlayer.team.name}</p>
                        </div>
